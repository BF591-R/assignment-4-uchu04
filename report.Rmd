...
caption_4 <- 'Figure 1. Scree plot of principal component variance. Bars show the proportion of variance explained by individual PCs and the overlaid line represents the cumulative variance explained. The plot indicates how many PCs are needed to capture the majority of variation in gene expression data.'

caption_5 <- 'Figure 2. Biplot of PC1 and PC2. Each point represents an individual sample that is of C3 or C4 subtype. PC1 and PC2 capture the two greatest sources of variance in intensity data, with percentage of variance indicated on each axis. Clustering of same-colored points indicates similar expression profiles within subtypes'

caption_8 <- 'Figure 3. Heatmap of normalized intensity values. Each row represents a significant probe (padj < 0.01) and each column represents an individual sample (GSM ID). Color intensity reflects normalized expression values, red indicating higher expression and blue indicating lower expression.'
```
...
variance_tibble %>%
  ...
  scale_color_manual(name="Cumulative",values=ls_cols) +
